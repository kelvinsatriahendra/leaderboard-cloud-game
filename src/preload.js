...
class Preload extends Phaser.Scene {
  ...
  preload() {
    ...
    progressBox.fillRoundedRect(
      this.width / 2 - progressBoxWidth / 2,
    ...
    const loadingText = this.add
      .text(this.width / 2, this.height / 2 + 50, "Memuat ...", {
        // Geser dikit ke bawah biar ga nabrak logo
        fontSize: "30px",
        fill: "#ffffff",
    ...
    this.load.on("progress", (value) => {
      ...
      progressBar.fillRoundedRect(
        this.width / 2 - progressBarWidth / 2,
    ...
    // Memuat Logo Lari Trus
    this.load.image("logoBaru", "../assets/logoLariTrus.png");

    // Asset lama tetap ada (jika masih dipakai di menu lain)
    ...
    // --- PLAYER TEXTURE ---
    this.load.image("kentaro", "../assets/kentaro.png", {
      // frameWidth: 50,
      // frameHeight: 60,
    });
  ...
  create() {
    // Menampilkan Logo Lari Trus di tengah layar (sedikit ke atas)
    this.add
      .image(this.width / 2, this.height / 2 - 50, "logoBaru")
      .setScale(0.4);

    this.message = this.add
      .text(
        this.scale.width / 2,
        this.scale.height - 100, // Taruh tulisan di bagian bawah
        'SENTUH LAYAR ATAU TEKAN "ENTER" UNTUK MENU UTAMA', // Saya update teksnya sedikit biar jelas
        {
          fontSize: "25px",
          fill: "#ffffff",
          fontFamily: '"Akaya Telivigala"',
    ...
    // --- [BARU] INPUT SENTUH (TOUCHSCREEN) ---
    // Tambahkan ini agar bisa di-tap di HP
